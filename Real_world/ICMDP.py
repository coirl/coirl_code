...
class Features:
    ...
    def mat_form(self, num_actions = 0,action_features=False):
        if num_actions == 0:
            ...
            if action_features:
                mat = np.zeros([self.__num_states* num_actions, self.__dim_features+num_actions])
                for i in range(self.__num_states):
                    for j in range(self.__dim_features+num_actions):
                        for k in range(num_actions):
                            if j < self.__dim_features:
                                mat[i*num_actions + k,j] = self.__features[i][j]
                            else:
                                aa = 0
                                if j - self.__dim_features == k:
                                    aa = 1
                                mat[i*num_actions + k,j] = aa
            else:
...
class MDP_SOL:
    ...
    # description:
    #   initiates features as an empty array in a specified size or as the given state-feature mapping
    def __init__(self, state_feature_exp, policy, M, Q=empty(),sample_len=empty(),stoch_pol=empty()):
...
class ICMDP:
    ...
    ##################################################
    def set_THETA_mat(self, T_mat):
        self.__THETA_mat = T_mat.copy()
    ...
    def solve_MDP(self, gamma, tol = 0.001, w=empty(), flag = 'init', state_feature_exp = empty(),init_state=0,length=0,top_policy=0,FH=False,action_features=False,stochastic=False):
        ...
        if isinstance(self.__F_mat,empty):
            self.__F_mat = self.__F.mat_form(num_actions=self.__num_actions,action_features=action_features)
            self.__F = 0
        ...
        if (isinstance(state_feature_exp,empty)):
            ...
            if action_features:
                new_state_feature_exp = np.zeros([self.__num_states, self.__dim_features+self.__num_actions])
            else:
        ...
        #print(w.shape)
        #print(new_state_feature_exp.shape)
        ...
        if FH:
            FW = np.matmul(self.__F_mat,w)
            SW = np.matmul(new_state_feature_exp,w)
            policy = np.random.randint(self.__num_actions,size=self.__num_states)
        ...
        while (delta > tol):# or (num_iter < 100):
            ...
            if FH:
                ...
                for itit in range(FH):
                    Q_w = FW + gamma *np.matmul(self.__THETA_mat , SW)
        ...
        if FH:
            ...
            for it in range(FH):
                ...
                new_state_feature_exp = Q[policy_ind,:].copy()
        if stochastic:
            stoch_pol = softmax(np.reshape(Q_w,[self.__num_states,self.__num_actions]), axis=1)
        # return the feature expectations according to initial state distribution (uniform or a specific initial state):
        if top_policy > 0:
            policy_i = policy.copy()
            sorted_p = np.argsort( -1 * np.reshape(Q_w, [self.__num_states, self.__num_actions]), axis=1)
            sorted = -1 * np.sort( -1 * np.reshape(Q_w, [self.__num_states, self.__num_actions]), axis=1)
            policy = np.reshape(sorted_p[:,:top_policy],[self.__num_states,top_policy])
            # print(sorted_p[:,:top_policy].shape)
            pp_a = np.reshape(Q_w, [self.__num_states, self.__num_actions])
            # print(pp_a[np.arange(self.__num_states),policy_i].shape)
            # print(pp_a[np.arange(self.__num_states),policy[:,0]].shape)
            # print((policy_i != policy[:, 0]).shape)
            # print(policy_i)

            sanity_c = np.sum(np.multiply(policy_i != policy[:,0],(pp_a[np.arange(self.__num_states),policy[:,0]] - pp_a[np.arange(self.__num_states),policy_i])))
            if sanity_c != 0:
                print("!!!!! \nProblem, sanity_c = ",sanity_c)
        if flag == 'init':
        ...
        if length == 0:
            if stochastic:
                return MDP_SOL(new_state_feature_exp, policy, M,Q,stoch_pol=stoch_pol)
...
##################################################
        ...
        if (deep_policy):
            ...
            if one_hot == 1:
                ...
                policy = np.asarray(policy_model.predict(
                    np.asarray([np.append(np.outer(context,self.__F_mat[state*self.__num_actions]), ff_arr[state]) for state in np.arange(self.__num_states)])))
            else:
    ...
    def get_policy_for_train(self,policy,context,one_hot=0,init_state=0,length = 0,pca_=0,traj=0):
        inputs = []
        outputs = []
        if init_state < 0:
        ...
        for state in states:
            if one_hot==1:
                ...
                inputs.append(np.append(np.outer(context, self.__F_mat[state*self.__num_actions]), self.__secondary_features[state]))
            else:
